...
class Dashboard {
            ...
            <div class="wsr-section">
                ...
                <div class="wsr-actions">
                    <button id="wsr-flush-btn" class="button button-primary wsr-btn">
                        <span class="dashicons dashicons-trash" style="margin-top:2px;"></span> <?php esc_html_e( 'Flush All Cache', 'wp-static-runtime' ); ?>
                    </button>
                    <button id="wsr-crawl-start-btn" class="button wsr-btn">
                        <span class="dashicons dashicons-networking" style="margin-top:2px;"></span> <?php esc_html_e( 'Start Crawler', 'wp-static-runtime' ); ?>
                    </button>
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=wsr-settings' ) ); ?>" class="button wsr-btn">
                        <span class="dashicons dashicons-admin-settings" style="margin-top:2px;"></span> <?php esc_html_e( 'Settings', 'wp-static-runtime' ); ?>
                    </a>
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=wsr-diagnostic' ) ); ?>" class="button wsr-btn">
                        <span class="dashicons dashicons-search" style="margin-top:2px;"></span> <?php esc_html_e( 'Diagnostic', 'wp-static-runtime' ); ?>
                    </a>
                </div>
    ...
    private static function status_bar( $settings ) {
        ...
        $label = $on
            ? '<span class="dashicons dashicons-yes-alt" style="vertical-align:text-bottom;"></span> ' . __( 'Cache Active — Serving static HTML', 'wp-static-runtime' )
            : '<span class="dashicons dashicons-warning" style="vertical-align:text-bottom;"></span> ' . __( 'Cache Disabled — Enable in Settings', 'wp-static-runtime' );
        echo '<div class="' . esc_attr( $class ) . '">' . wp_kses_post( $label ) . '</div>';
    }

    private static function status_row( $label, $on ) {
        $badge = $on
            ? '<span class="wsr-badge ok"><span class="dashicons dashicons-yes-alt" style="font-size:12px;line-height:1;margin-right:2px;vertical-align:text-top;"></span>' . esc_html__( 'ACTIVE',   'wp-static-runtime' ) . '</span>'
            : '<span class="wsr-badge off"><span class="dashicons dashicons-dismiss" style="font-size:12px;line-height:1;margin-right:2px;vertical-align:text-top;"></span>' . esc_html__( 'INACTIVE', 'wp-static-runtime' ) . '</span>';
