...
class Settings {
    ...
    public static function render(): void {
        if ( isset( $_POST['wsr_save_settings'] ) ) {
            ...
            self::save();
            echo '<div class="notice notice-success"><p>' . esc_html__( 'Settings saved.', 'wp-static-runtime' ) . '</p></div>';
        }

        $settings = get_option( 'wsr_settings', \WSR\Constants::defaults() );
        ?>
        <div class="wrap wsr-wrap">
            <h1>⚙️ <?php esc_html_e( 'WP Static Runtime — Settings', 'wp-static-runtime' ); ?></h1>
            ...
            <form method="post" action="">
                ...
                <p class="submit">
                    <input type="submit" name="wsr_save_settings" class="button button-primary"
                        value="<?php esc_attr_e( 'Save Settings', 'wp-static-runtime' ); ?>" />
                </p>
            </form>
    ...
    private static function render_server_config(): void {
        ...
        <!-- ── Apache .htaccess ───────────────────────────────────────────── -->
        <div class="wsr-settings-section">
            <h2>🗂️ <?php esc_html_e( 'Apache — .htaccess Rules', 'wp-static-runtime' ); ?></h2>
        ...
        <!-- ── Nginx ──────────────────────────────────────────────────────── -->
        <div class="wsr-settings-section">
            <h2>🌐 <?php esc_html_e( 'Nginx — Server Block Snippet', 'wp-static-runtime' ); ?></h2>
            <p>
            ...
            <div style="background:#f0fdf4;border-left:4px solid #16a34a;padding:12px 16px;border-radius:4px;margin-bottom:12px;font-size:13px;">
                <strong>📋 <?php esc_html_e( 'How to use:', 'wp-static-runtime' ); ?></strong><br>
