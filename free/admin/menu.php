...
/**
 * Admin Menu — registers all WP Static Runtime admin pages.
 */
class Menu {
    ...
    public function register_pages() {
        ...
        add_submenu_page( 'wsr-dashboard', __( 'Dashboard',  'wp-static-runtime' ), '<span class="wsr-menu-item wsr-menu-dashboard">'  . __( 'Dashboard',  'wp-static-runtime' ) . '</span>', 'manage_options', 'wsr-dashboard',  [ Dashboard::class,  'render'         ] );
        add_submenu_page( 'wsr-dashboard', __( 'Cache',      'wp-static-runtime' ), '<span class="wsr-menu-item wsr-menu-cache">'      . __( 'Cache',      'wp-static-runtime' ) . '</span>', 'manage_options', 'wsr-cache',      [ Cache_Page::class, 'render'         ] );
        add_submenu_page( 'wsr-dashboard', __( 'Crawler',    'wp-static-runtime' ), '<span class="wsr-menu-item wsr-menu-crawler">'    . __( 'Crawler',    'wp-static-runtime' ) . '</span>', 'manage_options', 'wsr-crawler',    [ Cache_Page::class, 'render_crawler' ] );
        add_submenu_page( 'wsr-dashboard', __( 'Settings',   'wp-static-runtime' ), '<span class="wsr-menu-item wsr-menu-settings">'   . __( 'Settings',   'wp-static-runtime' ) . '</span>', 'manage_options', 'wsr-settings',   [ Settings::class,   'render'         ] );
        add_submenu_page( 'wsr-dashboard', __( 'Diagnostic', 'wp-static-runtime' ), '<span class="wsr-menu-item wsr-menu-diagnostic">' . __( 'Diagnostic', 'wp-static-runtime' ) . '</span>', 'manage_options', 'wsr-diagnostic', [ Diagnostic::class, 'render'         ] );
    }
    ...
    public function admin_bar_button( $bar ) {
        ...
        $bar->add_node( [
            ...
            'meta'  => [ 'title' => 'WP Static Runtime: Flush All Cache' ],
        ] );
    ...
    public function handle_flush_all() {
        ...
        wp_redirect( wp_get_referer() ?: admin_url( 'admin.php?page=wsr-dashboard' ) );
        exit;
    }
    ...
    public function handle_purge_page() {
        ...
        if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Unauthorized' );
        $url = esc_url_raw( isset( $_GET['url'] ) ? $_GET['url'] : '' );
        if ( $url ) \WSR\Cache_Cleaner::purge_url( $url );
        wp_redirect( wp_get_referer() ?: admin_url( 'admin.php?page=wsr-cache' ) );
        exit;
    }

    public function show_reinstalled_notice() {
        if ( isset( $_GET['reinstalled'] ) && $_GET['page'] === 'wsr-diagnostic' ) {
            echo '<div class="notice notice-success is-dismissible"><p>✅ <strong>WP Static Runtime:</strong> Reinstalled successfully.</p></div>';
        }
    }
...
/* ── WP Static Runtime: Sidebar Submenu Icons ─────────────────────────────── */
...
.wsr-menu-dashboard::before  { -webkit-mask-image: <?php echo $encoded['dashboard'];  ?>; mask-image: <?php echo $encoded['dashboard'];  ?>; }
.wsr-menu-cache::before      { -webkit-mask-image: <?php echo $encoded['cache'];      ?>; mask-image: <?php echo $encoded['cache'];      ?>; }
.wsr-menu-crawler::before    { -webkit-mask-image: <?php echo $encoded['crawler'];    ?>; mask-image: <?php echo $encoded['crawler'];    ?>; }
.wsr-menu-settings::before   { -webkit-mask-image: <?php echo $encoded['settings'];   ?>; mask-image: <?php echo $encoded['settings'];   ?>; }
.wsr-menu-diagnostic::before { -webkit-mask-image: <?php echo $encoded['diagnostic']; ?>; mask-image: <?php echo $encoded['diagnostic']; ?>; }
