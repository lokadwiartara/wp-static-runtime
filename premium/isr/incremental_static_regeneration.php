...
class ISR {
    ...
    public static function process_queue(): void {
        ...
        if ( ! \WSR\Premium\License\License_Guard::verify() ) {
            error_log( '[WSR ISR] process_queue blocked — license invalid.' );
            return;
        }
    ...
    public static function revalidate_url( string $url ): bool {
        // Layer 2: License gate inside core function
        if ( ! \WSR\Premium\License\License_Guard::verify() ) {
            error_log( '[WSR ISR] revalidate_url blocked — license invalid.' );
            return false;
        }
        ...
        $response = wp_remote_get( $url, [
            ...
            'user-agent' => 'WP-Static-Runtime-ISR/1.0',
            'headers'    => [
                'X-WSR-ISR'     => '1',
        ...
        // Atomic rename (POSIX — same filesystem)
