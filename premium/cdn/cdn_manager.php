...
class CDN_Manager {
    ...
    public static function purge_url( string $url ): array {
        ...
        if ( ! \WSR\Premium\License\License_Guard::verify() ) {
            error_log( '[WSR CDN] purge_url blocked — license invalid.' );
            return [];
        }
        ...
        foreach ( self::$providers as $provider ) {
            try {
                ...
                error_log( '[WSR CDN] ' . $provider->name() . ' purge_url error: ' . $e->getMessage() );
            }
        }
    ...
    public static function purge_urls( array $urls ): array {
        // Layer 2: License gate inside core function
        if ( ! \WSR\Premium\License\License_Guard::verify() ) {
            error_log( '[WSR CDN] purge_urls blocked — license invalid.' );
            return [];
        }
    ...
    public static function purge_all(): array {
        // Layer 2: License gate inside core function
        if ( ! \WSR\Premium\License\License_Guard::verify() ) {
            error_log( '[WSR CDN] purge_all blocked — license invalid.' );
            return [];
        }
        ...
        foreach ( self::$providers as $provider ) {
            try {
                ...
                $results[ $provider->name() ] = [ 'success' => false, 'message' => $e->getMessage() ];
                error_log( '[WSR CDN] ' . $provider->name() . ' purge_all error: ' . $e->getMessage() );
            }
        }
