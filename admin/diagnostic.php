...
class Diagnostic {
    ...
    public static function render() {
        ...
        <div class="wrap wsr-wrap">
            <h1>🔍 <?php esc_html_e( 'Diagnostic', 'statixpress-static-runtime' ); ?></h1>
            <p><?php esc_html_e( 'This page checks every part of WP Static Runtime to find what\'s not working.', 'statixpress-static-runtime' ); ?></p>

            <?php if ( count( $fail ) === 0 ) : ?>
                <div class="wsr-status-bar active">✅ All checks passed – plugin is fully operational.</div>
            <?php else : ?>
                <div class="wsr-status-bar inactive">
                    ⚠️ <?php echo count( $fail ); ?> issue(s) found – see details below.
                </div>
            <?php endif; ?>
                ...
                <table class="widefat wsr-status-table">
                    <thead>
                        <tr>
                            ...
                            <th><?php esc_html_e( 'Check', 'statixpress-static-runtime' ); ?></th>
                            <th><?php esc_html_e( 'Value / Status', 'statixpress-static-runtime' ); ?></th>
                            <th><?php esc_html_e( 'Fix', 'statixpress-static-runtime' ); ?></th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <div class="wsr-section">
                <h2><?php esc_html_e( 'Manual Cache Test', 'statixpress-static-runtime' ); ?></h2>
                <p><?php esc_html_e( 'Click the button below to manually cache the homepage. Check your cache directory afterwards.', 'statixpress-static-runtime' ); ?></p>
                <div class="wsr-actions">
                    <button id="wsr-test-cache-btn" class="button button-primary wsr-btn">
                        🧪 <?php esc_html_e( 'Test: Cache Homepage Now', 'statixpress-static-runtime' ); ?>
                    </button>
                    <a href="<?php echo esc_url( admin_url( 'admin-post.php?action=wsr_reinstall&_wpnonce=' . wp_create_nonce('wsr_reinstall') ) ); ?>"
                       class="button wsr-btn" onclick="return confirm('Reinstall advanced-cache.php and recheck wp-config?')">
                        🔧 <?php esc_html_e( 'Reinstall advanced-cache.php', 'statixpress-static-runtime' ); ?>
                    </a>
                </div>
            ...
            <div class="wsr-section">
                <h2><?php esc_html_e( 'Cache Directory', 'statixpress-static-runtime' ); ?></h2>
                <?php
                ...
                ?>
                <p>
                    <strong><?php esc_html_e( 'Directory:', 'statixpress-static-runtime' ); ?></strong>
                    <code><?php echo esc_html( $cache_dir ); ?></code>
                    <?php if ( ! is_dir( $cache_dir ) ) : ?>
                        <span class="wsr-badge off"><?php esc_html_e( 'Does not exist', 'statixpress-static-runtime' ); ?></span>
                    <?php elseif ( ! wp_is_writable( $cache_dir ) ) : ?>
                        <span class="wsr-badge off"><?php esc_html_e( 'Not writable', 'statixpress-static-runtime' ); ?></span>
                    <?php else : ?>
                        <span class="wsr-badge ok"><?php esc_html_e( 'Exists & writable', 'statixpress-static-runtime' ); ?></span>
                    <?php endif; ?>
                </p>
                <?php if ( empty( $files ) ) : ?>
                    <p style="color:#94a3b8;"><?php esc_html_e( 'No cached files found yet.', 'statixpress-static-runtime' ); ?></p>
                <?php else : ?>
                    <p><?php echo count( $files ); ?> <?php esc_html_e( 'HTML files found:', 'statixpress-static-runtime' ); ?></p>
    ...
    private static function run_all_checks() {
        ...
        $checks[] = [
            ...
            'ok'    => $ac_is_ours,
            'value' => ! $ac_exists ? __( 'missing', 'statixpress-static-runtime' ) : ( $ac_is_ours ? 'OK (WP Static Runtime)' : __( 'belongs to another plugin', 'statixpress-static-runtime' ) ),
        ...
        $dir_writable   = $dir_exists && wp_is_writable( $cache_dir );
        $checks[] = [
        ...
        $checks[] = [
            ...
            'value' => $table_exists ? __( 'exists', 'statixpress-static-runtime' ) : __( 'missing', 'statixpress-static-runtime' ),
        ...
        $checks[] = [
            ...
            'value' => $logged_in
                        ...
                        : __( 'No', 'statixpress-static-runtime' ),
        ...
        if ( is_dir( WSR_CACHE_DIR ) ) {
            ...
            if ( $can_write ) wp_delete_file( $test_file );
        }
        $checks[] = [
    ...
    public static function handle_reinstall() {
        ...
        wp_safe_redirect( admin_url( 'admin.php?page=wsr-diagnostic&reinstalled=1' ) );
        exit;
    }
}
