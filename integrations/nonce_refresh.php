...
class Nonce_Refresh {
    ...
    public static function handle_refresh(): void {
        ...
        $raw_actions = isset( $_GET['actions'] ) ? (array) map_deep( wp_unslash( $_GET['actions'] ), 'sanitize_key' ) : [];
...
(function () {
    ...
    var GLOBALS      = <?php echo $globals_json; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>;
    var _fetching    = null;
    var _cache       = {};
    ...
    private static function get_client_ip(): string {
        foreach ( [ 'HTTP_CF_CONNECTING_IP', 'HTTP_X_FORWARDED_FOR', 'REMOTE_ADDR' ] as $h ) {
            if ( ! empty( $_SERVER[ $h ] ) ) {
                $raw_ip = sanitize_text_field( wp_unslash( $_SERVER[ $h ] ) );
                return explode( ',', $raw_ip )[0];
            }
        }
        return 'unknown';
    }
