...
class Nginx {
    ...
    public static function generate_config(): string {
        ...
        $doc_root = rtrim( isset( $_SERVER['DOCUMENT_ROOT'] ) ? sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ) : ABSPATH, '/' );
        ...
        $conf .= "    add_header X-Cache-Engine \"statixpress-static-runtime\" always;\n";
        $conf .= "    if (\$wsr_skip = 0) {\n";
    ...
    private static function get_cache_path_relative_to_docroot(): string {
        ...
        $doc_root  = isset( $_SERVER['DOCUMENT_ROOT'] ) ? realpath( sanitize_text_field( wp_unslash( $_SERVER['DOCUMENT_ROOT'] ) ) ) : ABSPATH;
        if ( ! $doc_root ) {
            $doc_root = ABSPATH;
        }
    ...
    private static function is_nginx(): bool {
        return isset( $_SERVER['SERVER_SOFTWARE'] )
            && stripos( sanitize_text_field( wp_unslash( $_SERVER['SERVER_SOFTWARE'] ) ), 'nginx' ) !== false;
    }
}